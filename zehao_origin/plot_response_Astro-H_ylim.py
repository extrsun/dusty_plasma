# -*- coding: utf-8 -*-
"""
Created on Mon Jul 24 21:45:02 2017
...
#######just delete this if te is not in 1.0,2.0,4.0,10.0
if te==1.0:
    plt.ylim(0,6e-25)
if te==2.0:
    plt.ylim(0,6e-23)
...
plt.savefig('results/response/v%s_%skeV_Astro-H_%sdust_Raw_input_data:Energy_vs_Emissivity_y.eps' %(str(version),str(te),str(dust_perc)))
...
plt.savefig('results/response/v%s_%skeV_Astro-H_%sdust_Processed_input_data:Energy_vs_Counts_y.eps' %(str(version),str(te),str(dust_perc)))
...
plt.ylabel('Spectrum $({counts* \mathrm{cm}^3 \mathrm{s}^{-1}} \mathrm{keV}^{-1})$')
plt.savefig('results/response/v%s_%skeV_Astro-H_%sdust_after_responsing_by_Astro-H_SXS_y.eps' %(str(version),str(te),str(dust_perc)))
plt.show()
